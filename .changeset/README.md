...
`@lensflare/effect` follows the workspace Effect catalog version. The version command runs `pnpm sync:effect-sdk-version` after Changesets updates changelogs, so the package version and `effect` peer dependency stay aligned with `pnpm-workspace.yaml`.
